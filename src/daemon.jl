...
function start(d::Daemon)
    ...
    p            = run(Cmd(`$julia_exec -t auto --project=$project_path -e "using DFControl; DAEMON = DFControl.server_start($(d.port)); using DaemonMode; serve($(d.port), true)"`; detach = true); wait = false)
    d.pid        = getpid(p)
